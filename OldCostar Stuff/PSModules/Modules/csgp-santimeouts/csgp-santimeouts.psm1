...
arrays we will set all timeouts back to thier defaults.
...
those timeout settings.  we need to revert those settings.
...
.EXAMPLE
	$env:PSModulePath = "\\dcfile1\systems\scripts\Modules;C:\Windows\System32\WindowsPowerShell\v1.0\Modules"
    get-alltimeouts -computer [computername]
.EXAMPLE
	$env:PSModulePath = "\\dcfile1\systems\scripts\Modules;C:\Windows\System32\WindowsPowerShell\v1.0\Modules"
    set-alltimeouts -computer [computername]
#>
...
function set-alltimeouts
{
	...
	PROCESS
	{
		try
		{
			...
			set-DiskTimeout -computer $computer
			set-MPIOTimeout -computer $computer
			set-LinkDownTimeResult -computer $computer
			set-MaxRequestHoldTime -computer $computer
...
function Test-mpio
{
  ...
  #>
	...
	process
	{
		...
		{
			write-verbose "$computer has Server 2003 OS, not supported for MPIO"
			Return
		}
		...
		if ($mpioStartValue -eq 0)
		{
			[boolean]$mpoiInstalled = $true
			Write-Verbose "$computer has mpio installed"
...
function get-tcpdelayedack
{
  ...
  #>
	...
	process
	{
		...
		# TcpAckFrequency may be missing from one of the two nics in the  registry so catch() that.
		try
		{
			$result.ValueData | Out-Null
		...
		{
			Write-warning "Failed checking $valname on $computer\$guid"
			$hasPassed = $false
			$valueData = 'missing'
		...
		$obj = @()
		$obj = "" | Select-Object Computer, ValueName, ValueData, ValueKind, Pass, CorrectValue
		$obj.computer = $computer
		$obj.ValueName = $valName
		$obj.ValueData = $valueData
		$obj.ValueKind = 'DWord'
		$obj.pass = $hasPassed
		$obj.CorrectValue = $valData
		#$result | Add-Member -type NoteProperty -name Pass -value $hasPassed
