...
If there are no subkeys Enum is the output
...
[CmdletBinding(supportsshouldprocess=$true)]
	...
	PROCESS	{
		...
		if (! $regKey) { 
			write-error "${computer}:key $key not found or error occurred"
		}
...
<#
.SYNOPSIS
get-csgpRegistryKeyValue will connect to a remote (or local) computer and list
all registry values in the given key.  If you want to find a specific value do 
not specify the value (like a Dword value) in the -key name, just supply actual
keys with -key.
...
.EXAMPLE
import-module csgp-registry -force; 
get-csgpRegistryKeyValue -computer dcadmin8 -registryHive LocalMachine -key "SOFTWARE\Microsoft\Windows\CurrentVersion" -registryHive LocalMachine

.EXAMPLE
import-module csgp-registry -force; 
get-csgpRegistryKeyValue -computer dcadmin8 -registryHive LocalMachine -key "SYSTEM\CurrentControlSet\services\Tcpip\Parameters" 

.EXAMPLE
import-module csgp-registry -force; 
get-csgpRegistryKeyValue -computer visqlprd560 -registryhive LocalMachine -key "SYSTEM\CurrentControlSet\Services\Disk"
